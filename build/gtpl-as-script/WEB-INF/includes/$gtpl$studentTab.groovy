...
out.print("""
		<div id=\"student_tab\">
		  <div class=\"new_record_form ui-corner-all\">
		    """);
/* include#begin \WEB-INF\includes\addStudent.gtpl */
...
<script type=\"text/javascript\">
  ...
  function openAddStudentDialogClassAttended() {
		...
		jQuery.ajax(
			{
				...
				, data: jQuery.param(
						{
							isMatchingList: \"Y\"
							, firstName: newStudentForm.find( \".add_student_first_name\" ).val()
							, lastName: newStudentForm.find( \".add_student_last_name\" ).val()
							, birthDate: newStudentForm.find( \".add_student_birth_date\" ).val()
							, village: newStudentForm.find( \".add_student_village\" ).val()
							, limit: 5
							, offset: 0
						}
					)
				...
				, success: function( data ) {
						if( jQuery( data ).find( \".list_student_details\" ).length > 0 ) {
							var addStudentDuplicateListStudent = jQuery( \".add_student_matching_list_student\" );
							addStudentDuplicateListStudent.dialog( \"open\" );
							addStudentDuplicateListStudent.children().remove();
							addStudentDuplicateListStudent.append( data );
						}
						else {
...
/* include#end   \WEB-INF\includes\addStudent.gtpl */
out.print("""
		  </div>
		
		  <div class=\"new_record_radio\">
		        <input type=\"radio\" id=\"add_student_radio\" name=\"new_student_radio\" checked=\"checked\" /><label for=\"add_student_radio\">add new</label>
		        <input type=\"radio\" id=\"upload_students_radio\" name=\"new_student_radio\" /><label for=\"upload_students_radio\">upload</label>
		  </div>
		  ...
		  <div class=\"list_record_form ui-corner-all\">
		    """); include '/WEB-INF/includes/listStudent.gtpl?limit=20&offset=0' ;
out.print("""
		  </div>
		
		  <div class=\"list_record_radio\">
		        <input type=\"radio\" id=\"list_students_radio\" name=\"list_student_radio\" checked=\"checked\" /><label for=\"list_students_radio\">list</label>
		        <input type=\"radio\" id=\"download_students_radio\" name=\"list_student_radio\" /><label for=\"download_students_radio\">download</label>
		  </div>
		</div>
"""); } ;
out.print("""
...
<script type=\"text/javascript\">
  ...
  var middleFilterSortBy = jQuery( \"#student_tab\" ).find( \".middle_filter, .middle_sortby\" );
          ...
          middleFilterSortByDialog.dialog( \"option\", \"position\",
          	{
          		...
          		, of: jQuery( \"#student_tab .middle_sortby\" )
          		, collision: \"fit\"
          	}
          );
  ...
  var listRecordRadio = jQuery( \"#student_tab .list_record_radio\" ).buttonset();
  ...
  var newRecordRadio = jQuery( \"#student_tab .new_record_radio\" ).buttonset();
