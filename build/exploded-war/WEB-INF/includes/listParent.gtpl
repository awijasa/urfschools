<%
  if( user == null ) redirect "/index.gtpl"
	  ...
	  if( params.parentId != null && params.isLookup == null ) {
	    ...
	    List<com.google.appengine.api.datastore.Entity> parentList = data.Parent.findByLimitAndOffset( limit, offset )
	  ...
	  if( offset > 0 ) {
		...
		html.div( class: "parent_list" ) {
			if( params.isLookup == "Y" ) {
				...
				mkp.yieldUnescaped( formatter.ListItemFormatter.getParentListItem( data.Parent.findByParentId( params.parentId ) ) )
			}
				...
				data.Relationship.findByStudentIdAndLimitAndOffset( params.studentId, limit, offset ).eachWithIndex(
					{ obj, i ->
						...
						if( i < limit )
							mkp.yieldUnescaped( formatter.ListItemFormatter.getParentListItem( parent ) )
					}
				)
			}
			else {
				...
				parentList.eachWithIndex(
					{ obj, i ->
						if( i < limit )
							mkp.yieldUnescaped( formatter.ListItemFormatter.getParentListItem( obj ) )
					}
				)
			}
		}
