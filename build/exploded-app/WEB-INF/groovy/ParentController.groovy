...
if( URFUser.findByEmail( user?.getEmail() ) == null && !localMode ) {
	...
	if( params.action == "delete" ) {
		...
		try {
				...
				Parent.findByLimitAndOffset( 0, Integer.parseInt( params.nextTwentyOffset ) ).each(
					{
						 ...
						 */
						println ListItemFormatter.getParentListItem( it, null )
					}
				)
			}
			else {
				Relationship.findByStudentIdAndLimitAndOffset( params.studentId, 0, Integer.parseInt( params.nextTwentyOffset ) ).each(
					{
						/*
						 * Respond with the HTML code that is required to display the Parent entity that can fill the empty space that resulted
						 * from the deleted record within an accordion.
						 */
						println ListItemFormatter.getParentListItem( Parent.findByParentId( it.parentId ), params.studentId )
					}
				)
			}
			...
			if( Parent.isUsed( parent ) || ( ( params.studentId?: "" ) != "" && relationships.size() > 1 ) )
				throw new EntityIsUsedException( parent, Parent.getPrimaryKey() )
            ...
            */
            ...
            while( allParamNames.hasMoreElements() ) {
            	...
            	if( paramName.startsWith( "deleteChildId" ) || paramName.startsWith( "deleteRelativeId" ) ) {
            		...
            		String schoolName = StudentDocument.findByStudentId( studentId ).getOnlyField( "lastEnrollmentSchool" ).getText()
            		relationship = Relationship.findByParentIdAndStudentId( parent.parentId, studentId )
					
					if( Payment.findByFundingSourceAndStudentId( parent.parentId, studentId ).size() > 0 )
						throw new EntityIsUsedException( relationship, Relationship.getPrimaryKey() )
            		...
            		String schoolName = StudentDocument.findByStudentId( relationship.studentId ).getOnlyField( "lastEnrollmentSchool" ).getText()
            ...
            println ListItemFormatter.getParentListItem( parent, null )
		}
		catch( Exception e ) {
